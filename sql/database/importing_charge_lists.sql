...
LOAD DATA INFILE 'D:/Data Projects/NovantHealthPresbyterianMedicalCenter_standardcharges.csv' 
...
DELETE FROM novant_presbyterian_std 
WHERE 
    aetna_coventry IS NULL AND
    aetna_coventry_rental_network IS NULL AND
    aetna_coventry_mcr IS NULL AND
    alliance_behavioral_health IS NULL AND
    atlantic_packaging IS NULL AND
    bcbsnc_blue_medicare IS NULL AND
    bcbsnc_blue_value IS NULL AND
    bcbsnc_blue_home IS NULL AND
    bcbsnc_ppo_hmo IS NULL AND
    cardinal_innovations IS NULL AND
    carolina_complete IS NULL AND
    cigna IS NULL AND
    cigna_healthspring IS NULL AND
    healthy_blue IS NULL AND
    humana_commercial IS NULL AND
    humana_medicare IS NULL AND
    humana_ppox IS NULL AND
    humana_bh_commercial IS NULL AND
    humana_bh_mcr_advantage IS NULL AND
    liberty_advantage_mcr_hmo IS NULL AND
    medcost_mbs IS NULL AND
    medcost_non_mbs IS NULL AND
    multiplan IS NULL AND
    pace IS NULL AND
    partners_bhm_lme_mco IS NULL AND
    phcs IS NULL AND
    primary_physician_care IS NULL AND
    provider_select IS NULL AND
    three_rivers_provider_network IS NULL AND
    uhc IS NULL AND
    uhc_mcr_adv IS NULL AND
    wellcare IS NULL AND
    amerihealth_caritas IS NULL;